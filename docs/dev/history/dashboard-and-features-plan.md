...
> **Historical document**, kept for its reasoning. Layout references (`src/bot`, `src/web`, the nested frontend npm project) predate the v4.0 workspace restructure — see [architecture.md](../architecture.md) for the current tree and [decisions.md](../decisions.md) (v4.0 entries) for what changed and why. The reasoning recorded here still applies.
