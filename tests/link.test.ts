...
describe("logWatcher link handler", () => {
  ...
  it("links account on valid code", async () => {
    ...
    const mockClient = {
      users: {
        cache: new Map([
          ["discord-99", { send: vi.fn().mockResolvedValue(undefined) }],
        ]),
      },
    };
    await handler(
  ...
  it("rejects expired code and does not link", async () => {
    ...
    const mockClient = {
      users: { cache: new Map([["discord-42", { send }]]) },
    };
    await handler(
      "Steve",
      { code: "EXPIRED1" },
    ...
    for (const [linkedMap] of calls) {
      expect(
        (linkedMap as Record<string, string>)["discord-42"],
      ).toBeUndefined();
    }
  ...
  it("rate-limits rapid successive attempts from same player", async () => {
    ...
    const mockClient = {
      users: {
        cache: new Map([
          ["discord-77", { send: vi.fn().mockResolvedValue(undefined) }],
        ]),
      },
    };
    ...
    expect(vi.mocked(saveLinkedAccounts).mock.calls.length).toBe(
      callsAfterFirst,
    );
