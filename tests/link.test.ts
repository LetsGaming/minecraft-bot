...
describe('logWatcher link handler', () => {
  ...
  async function buildHandler() {
    // Mock persistence layer — code state lives only in module memory.
    ...
    // init() loads codes/linked from the mock; originalInit() registers the watcher.
    await mod.init();
    return mod;
  }

  it('links account on valid code', async () => {
    const { COMMAND_INFO } = await buildHandler();

    // Inject a fresh code entry directly into the module-level codes map.
    // We do this by going through the real loadLinkCodes mock return — the
    // module reads it on init. Instead, let's just exercise the handler
    // in isolation by importing the internal defineCommand result via a fresh module.
    //
    // Simpler approach: test that the handler does NOT return early for a valid code
    // by stubbing codes via the linkUtils mock.
    const { loadLinkCodes } = await import('../src/utils/linkUtils.js');
    const mockCodes = {
      ABCD1234: { discordId: 'discord-99', expires: Date.now() + 60_000, confirmed: false },
    };
    vi.mocked(loadLinkCodes).mockResolvedValue(mockCodes as never);

    // Reload the module so it picks up the new code map.
    vi.resetModules();
    const { COMMAND_INFO: ci, init } = await import('../src/logWatcher/commands/link.js');
    await init();

    const mockClient = { users: { cache: new Map([['discord-99', { send: vi.fn() }]]) } };
    await ci.handler('Steve', { code: 'ABCD1234' }, mockClient as never);
  ...
  it('ignores unknown code', async () => {
    ...
    const { COMMAND_INFO, init } = await import('../src/logWatcher/commands/link.js');
    await init();

    const mockClient = { users: { cache: new Map() } };
    await COMMAND_INFO.handler('Steve', { code: 'XXXXXXXX' }, mockClient as never);
  ...
  it('rejects expired code and does not link', async () => {
    const expiredCode = { discordId: 'discord-42', expires: Date.now() - 1000, confirmed: false };
    vi.mock('../src/utils/linkUtils.js', () => ({
      loadLinkCodes: vi.fn().mockResolvedValue({ EXPIRED1: expiredCode }),
      loadLinkedAccounts: vi.fn().mockResolvedValue({}),
      saveLinkCodes: vi.fn().mockResolvedValue(undefined),
      saveLinkedAccounts: vi.fn().mockResolvedValue(undefined),
    }));
    ...
    await COMMAND_INFO.handler('Steve', { code: 'EXPIRED1' }, mockClient as never);
  ...
  it('rate-limits rapid successive attempts from same player (F-002)', async () => {
    const validCode = { discordId: 'discord-77', expires: Date.now() + 60_000, confirmed: false };
    vi.mock('../src/utils/linkUtils.js', () => ({
      loadLinkCodes: vi.fn().mockResolvedValue({ VALID001: validCode }),
      loadLinkedAccounts: vi.fn().mockResolvedValue({}),
      saveLinkCodes: vi.fn().mockResolvedValue(undefined),
      saveLinkedAccounts: vi.fn().mockResolvedValue(undefined),
    }));
    ...
    const mockClient = { users: { cache: new Map([['discord-77', { send: vi.fn() }]]) } };

    // First attempt — should go through.
    await COMMAND_INFO.handler('Herobrine', { code: 'VALID001' }, mockClient as never);
    const { saveLinkedAccounts } = await import('../src/utils/linkUtils.js');
