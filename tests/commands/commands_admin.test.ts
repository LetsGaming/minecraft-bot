...
describe("/config admin command — reload subcommand", () => {
  ...
  it("reconciles server changes and reports them as applied live", async () => {
    ...
    const { createSuccessEmbed } = await import("../../src/bot/utils/embeds/embedUtils.js");
...
describe("logWatcher !seed command", () => {
  it("registers via init() without throwing", async () => {
    const { init } = await import("../../src/bot/logWatcher/commands/info/seed.js");
...
describe("logWatcher !netherportal command", () => {
  it("registers via init() without throwing", async () => {
    const { init } = await import("../../src/bot/logWatcher/commands/info/netherportal.js");
    expect(() => init()).not.toThrow();
  });
});
...
describe("logWatcher !chunkbase command", () => {
  it("registers via init() without throwing", async () => {
    const { init } = await import("../../src/bot/logWatcher/commands/info/chunkbase.js");
    expect(() => init()).not.toThrow();
  });
});
