...
// playerUtils is the module under test here — getListOutput and
// stripLogPrefix now live inside it (they used to sit in the utils.ts
// grab-bag and were stubbed from outside). Nothing else in this file
// depends on them, so the real implementations run.
...
describe("setupDiscordToMc", () => {
  ...
  beforeEach(async () => {
    ({ setupDiscordToMc } =
      await import("../../src/bot/logWatcher/watchers/log/chatBridge.js"));
  });
...
describe("playerUtils", () => {
  ...
  beforeEach(async () => {
    const mod = await import("../../src/core/utils/minecraft/playerUtils.js");
...
describe("downtimeMonitor — state machine", () => {
  ...
  beforeEach(async () => {
    ({ startDowntimeMonitor, suppressAlerts } =
      await import("../../src/bot/logWatcher/watchers/monitors/downtimeMonitor.js"));
    vi.clearAllMocks();
  });
