...
describe("validateCandidateConfig — basics", () => {
  ...
  // These numeric guards existed with no real coverage: tests/validateConfig
  // .test.ts claimed them, but copied the `if` into the test body and
  // asserted its own throw, so the validator could have dropped either check
  // with the suite green. Asserted against validateCandidateConfig here.
  it("rejects a tpsWarningThreshold that is not a positive number", () => {
    for (const bad of [-1, 0, "15"]) {
      const result = validateCandidateConfig({
        ...base,
        tpsWarningThreshold: bad,
      });
      expect(result.valid, `expected ${JSON.stringify(bad)} to be rejected`).toBe(
        false,
      );
      expect(result.errors.join("\n")).toContain("tpsWarningThreshold");
    }
    expect(
      validateCandidateConfig({ ...base, tpsWarningThreshold: 15 }).valid,
    ).toBe(true);
  });
});
