...
import { detectCapabilities } from "../../src/core/utils/server/serverAccess.js";
import {
  capabilityCommandSkips,
  capabilitySummary,
  requireCapability,
} from "../../src/core/utils/server/capabilities.js";
...
describe("deleteStatsFile — remote wrapper", () => {
  ...
  it("calls DELETE /stats/:uuid and returns the wrapper's verdict", async () => {
    const { deleteStatsFile } = await import("../../src/core/utils/server/serverAccess.js");
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
