...
import { flattenStats, filterStats } from "../../src/core/utils/minecraft/statUtils.js";
...
describe("flattenStats", () => {
  ...
  it("skips non-numeric values in nested format", () => {
    const file = {
      stats: {
        "minecraft:custom": {
          "minecraft:play_time": "bad" as unknown as number,
        },
      },
    };
    const flat = flattenStats(file);
...
describe("filterStats", () => {
  const stats = [
    {
      fullKey: "minecraft:mined.stone",
      category: "minecraft:mined",
      key: "stone",
      value: 5,
    },
    {
      fullKey: "minecraft:custom.deaths",
      category: "minecraft:custom",
      key: "minecraft:deaths",
      value: 2,
    },
    {
      fullKey: "minecraft:killed.zombie",
      category: "minecraft:killed",
      key: "zombie",
      value: 10,
    },
    {
      fullKey: "minecraft:killed_by.creeper",
      category: "minecraft:killed_by",
      key: "creeper",
      value: 3,
    },
  ];
  ...
  it('disambiguates "killed_by"', () => {
    ...
    expect(result.every((s) => s.category === "minecraft:killed_by")).toBe(
      true,
    );
  });
