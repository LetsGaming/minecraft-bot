...
async function handleNetherportalCommand(match) {
  ...
  try {
    ...
    if (!coords) {
      await sendToServer(
        `/tellraw ${username} {"text":"❌ Could not get your position. Make sure you're online.","color":"red"}`
      );
      return;
    }
    ...
    await sendToServer(
      `/tellraw ${username} {"text":"❌ Error calculating portal coordinates.","color":"red"}`
    );
  }
}
