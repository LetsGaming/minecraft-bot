...
} from "@mcbot/core/utils/minecraft/snapshotUtils.js";
...
async function checkAndPost(
  ...
  for (const [guildId, gcfg] of Object.entries(guildConfigs)) {
    ...
    const intervalMs = LEADERBOARD_INTERVAL_MS[interval];
    if (!intervalMs) {
      log.warn(
        "leaderboard",
    ...
    try {
      ...
      for (const server of servers) {
        ...
        if (snapshot) {
          ...
          const snapshotAge = Math.round((now - snapshot.timestamp) / HOUR_MS);

          // So users know it's a partial period when the bot is young:
          footer =
            snapshotAge > intervalMs / HOUR_MS
