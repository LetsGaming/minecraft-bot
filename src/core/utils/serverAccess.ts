...
/**
 * The oldest wrapper version this bot expects. The `/info` endpoint with
 * a `version` field plus the host metrics block below both land in this
 * wrapper release — the two halves of the roadmap's "remote host metrics
 * + version handshake" item ship together on the wrapper side.
 */
export const MIN_WRAPPER_VERSION = "1.2.0";
...
/**
 * Log a startup warning when a remote instance's wrapper is older than
 * the bot expects (or predates `/info` entirely). Never throws — a
 * mismatched wrapper degrades features, it must not stop the bot.
 */
export async function verifyWrapperVersion(cfg: ServerConfig): Promise<void> {
  if (!cfg.apiUrl) return;
  const info = await getRemoteInfo(cfg);
  if (!info || !info.version) {
    log.warn(
      cfg.id,
      `API wrapper does not report a version (/info missing or pre-1.2.0). ` +
        `The bot expects wrapper >= ${MIN_WRAPPER_VERSION}; remote host ` +
        `metrics and disk alerts are unavailable until the wrapper is updated.`,
    );
    return;
  }
