/**
 * Community waypoint persistence — single owner of data/waypoints.json;
...
const WAYPOINTS_PATH = path.resolve(getRootDir(), "data", "waypoints.json");
...
export async function loadWaypointStore(): Promise<WaypointStore> {
  const raw = await loadJson(WAYPOINTS_PATH);
...
export async function saveWaypointStore(store: WaypointStore): Promise<void> {
  return saveJson(WAYPOINTS_PATH, store);
}
