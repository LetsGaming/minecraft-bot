/**
 * Cross-platform polls — single owner of data/polls.json.
 *
...
const POLLS_PATH = path.resolve(getRootDir(), "data", "polls.json");
...
export async function loadPollStore(): Promise<PollStore> {
  const raw = await loadJson(POLLS_PATH);
...
export async function savePollStore(store: PollStore): Promise<void> {
  ...
  return saveJson(POLLS_PATH, store);
}
