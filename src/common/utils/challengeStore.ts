/**
 * Advancement challenges — single owner of data/challenges.json plus the
...
const CHALLENGES_PATH = path.resolve(getRootDir(), "data", "challenges.json");
...
export async function loadChallengeStore(): Promise<ChallengeStore> {
  const raw = await loadJson(CHALLENGES_PATH);
...
export async function saveChallengeStore(
  ...
  return saveJson(CHALLENGES_PATH, store);
}
