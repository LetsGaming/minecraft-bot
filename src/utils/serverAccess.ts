...
import { isSudoPermissionError, sudoHelpMessage } from '../shell/execCommand.js';
import type { ServerConfig, WhitelistEntry, MinecraftStatsFile } from '../types/index.js';
...
async function apiPost<T>(cfg: ServerConfig, route: string, body: unknown): Promise<T> {
  const url = `${cfg.apiUrl!.replace(/\/$/, '')}/instances/${cfg.id}${route}`;
...
export async function tailLog(cfg: ServerConfig, lines = 10): Promise<string> {
  if (cfg.apiUrl) {
    const { output } = await apiGet<{ output: string }>(cfg, `/logs/tail?lines=${lines}`);
    return output;
  }
...
export async function readWhitelist(cfg: ServerConfig): Promise<WhitelistEntry[]> {
  if (cfg.apiUrl) {
    const { whitelist } = await apiGet<{ whitelist: WhitelistEntry[] }>(cfg, '/whitelist');
    return whitelist;
  }
  try {
    const raw = await fsPromises.readFile(path.resolve(cfg.serverDir, 'whitelist.json'), 'utf-8');
...
export async function readLevelName(cfg: ServerConfig): Promise<string> {
  ...
  try {
    const text = await fsPromises.readFile(path.resolve(cfg.serverDir, 'server.properties'), 'utf-8');
...
export async function readStats(cfg: ServerConfig, uuid: string): Promise<MinecraftStatsFile | null> {
  if (cfg.apiUrl) {
    const { stats } = await apiGet<{ stats: MinecraftStatsFile | null }>(cfg, `/stats/${uuid}`);
    return stats;
  }
...
export async function deleteStatsFile(cfg: ServerConfig, uuid: string): Promise<boolean> {
  if (cfg.apiUrl) {
 ...
 */
export async function readModSlugs(cfg: ServerConfig): Promise<{ slugs: string[]; mtimeMs: number }> {
  if (cfg.apiUrl) {
    return apiGet<{ slugs: string[]; mtimeMs: number }>(cfg, '/mods');
  }
  const jsonPath = path.join(cfg.scriptDir, 'common', 'downloaded_versions.json');
  ...
  const raw = JSON.parse(fs.readFileSync(jsonPath, 'utf-8')) as { mods?: Record<string, unknown> };
...
export async function readBackups(cfg: ServerConfig): Promise<BackupSummary> {
  if (cfg.apiUrl) {
    const data = await apiGet<{
      dirs: Array<{ dir: string; count: number; latestFile: string; latestMtimeMs: number; latestSizeBytes: number }>;
      totalBytes: number;
    }>(cfg, '/backups');
    return {
      dirs: data.dirs.map(d => ({ ...d, latestMtime: new Date(d.latestMtimeMs) })),
      totalBytes: data.totalBytes,
    };
  }

  const backupsBase = path.resolve(cfg.serverDir, '..', 'backups', cfg.screenSession);
  ...
  for (const dir of subdirs) {
    ...
    dirs.push({ dir, count: files.length, latestFile: latest, latestMtime: stat.mtime, latestSizeBytes: stat.size });
  }
...
export async function runScript(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('sudo', ['-n', '-u', cfg.linuxUser, 'bash', scriptPath, ...args], {
      cwd: scriptDir,
...
export function logStreamUrl(cfg: ServerConfig): string {
  if (!cfg.apiUrl) throw new Error(`logStreamUrl called on local instance '${cfg.id}'`);
