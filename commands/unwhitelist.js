...
export const execute = withErrorHandling(requireServerAdmin(async (interaction) => {
  ...
  await recordRemove(username, interaction.user.tag, interaction.user.id, server.id);

  await interaction.editReply({ embeds: [createSuccessEmbed(`**${username}** has been removed from the whitelist on **${server.id}**.`)] });
}));
